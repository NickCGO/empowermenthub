  ...
  const fetchDashboardData = async () => {
    ...
    try {
      const [salesRes, topPerformersRes] = await Promise.all([
        fetch(`${backendUrl}/api/get-agent-sales/${userId}`, { headers: fetchHeaders }),
        fetch(`${backendUrl}/api/get-top-performers`, { headers: fetchHeaders })
      ]);
                ...
                {mySalesSummary ? (
                  <>
                      ...
                      <table className="min-w-full bg-white">
                        ...
                        <tbody><tr><td className="px-4 py-2 border-b">{mySalesSummary.period}</td><td className="px-4 py-2 border-b">{mySalesSummary.pending_sales}</td><td className="px-4 py-2 border-b">{mySalesSummary.confirmed_sales}</td><td className="px-4 py-2 border-b">{mySalesSummary.amount_earned}</td>
                          <td className="px-4 py-2 border-b">
                            <button onClick={handleRequestPayout} disabled={payoutLoading || mySalesSummary.confirmed_sales === 0} className="px-3 py-1 text-sm text-white rounded-md bg-navy-700 hover:bg-navy-800 disabled:bg-gray-400 disabled:cursor-not-allowed">
                    ...
                    <div className="mt-6">
                      ...
                      <div className="w-full h-2 bg-gray-200 rounded-full"><div className="h-2 rounded-full bg-navy-700" style={{ width: `${getLevelProgress(mySalesSummary.confirmed_sales)}%` }}></div></div>
